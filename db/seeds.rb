...
address = ['341 Upper St,London,N1 0PB','25 City Rd,London,EC1Y 1AA','91 City Road,London,EC1Y 1AF','1st Floor Leonard St,London,EC2A','87A Great Eastern Street,London,EC2A','127 91 City Road,London,EC1Y','Weymouth Terrace,London,E2','The South Penthouse,London,EC2Y','Duncan Terrace ,London,N1']

tenant_reviews = ["Had a great time in this flat.", "The place is very central and close to various amenities. I had a room with my own private bathroom and a shared kitchen where I could cook. The place is clean and well kept. I also high speed internet access.","Thank you, for making my stay so comfortable x","Very nice flatmate! The atmosphere is warm and friendly","He is an easygoing, willing and sociable","Fue de gran ayuda, siempre pendiente de lo que ncesitaba",'Ce proprio m\'a tant comble que je le recommande a toute celles ou tout ceux qui ont besoin de quelqu un sur qui compter']
property_reviews = ['The room we had was really a small small flat.','The management was very helpful.','This accommodation is the best location to stay in Old Street, we were in a quiet place and very close to the bus stops,','I really enjoyed my stay at this quiet homestay in the heart of London, I would recommend it especially for students','Mi estadia durante estos tres meses fue increible, el cuarto estaba impecable','La propiedad esta en perfecto estado, muy bien cuidada. 100% recomendada']
...
1.upto(10) do |n|
  property = Property.create(title: titles.sample, description: descriptions.sample, address: address.sample.split(",")[0] , city: address.sample.split(",")[1], postcode: address.sample.split(",")[2], total_rooms:[*1..3].sample.to_s)
  ...
  3.times { users.sample.reviews << Review.create(thoughts: tenant_reviews.sample, rating: [*1..5].sample, user_id: users.sample.id) }
  5.times{ property.reviews << Review.create(thoughts: tenant_reviews.sample, rating: [*1..5].sample, user_id: users.sample.id) }
	puts "#{n} uploaded"
end
